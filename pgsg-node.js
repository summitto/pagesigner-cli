...
async function printShares(host, date, notary_server_write_key_share, client_server_write_key_share){
    const sessDir = Path.join(__dirname, 'saved_sessions', date + "_" + host) 
    fs.mkdirSync(sessDir, { recursive: true });   
    fs.writeFileSync(Path.join(sessDir, "notary_server_write_key_share"), notary_server_write_key_share);
    fs.writeFileSync(Path.join(sessDir, "client_server_write_key_share"), client_server_write_key_share);
}

async function createNewSession(host, request, response, date, pgsg, notary_server_write_key_share, client_server_write_key_share, is_imported=false){
...
async function main (){
    ...
    if (argv[2] === 'notarize') {
        ...
        const notary_server_write_key_share = Buffer.from(obj['notary server_write_key share']).toString('hex');
        const client_server_write_key_share = Buffer.from(obj['client server_write_key share']).toString('hex');
  
        const sessDir = await createNewSession(host, request, response, date, serializedPgsg);
        await printShares(host, date, notary_server_write_key_share, client_server_write_key_share);
